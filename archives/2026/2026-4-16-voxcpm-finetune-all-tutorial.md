...
TRAIN_OUTPUT = "train_data.jsonl"     # 输出的训练集文件名（脚本同目录下）

# 验证集输出路径：支持两种写法
#   1) 仅文件名（如 "val_data.jsonl"）→ 落到脚本同目录下（默认行为，与原版一致）
#   2) 绝对路径（如 r"D:\tts\val\val_data.jsonl" 或 "/root/autodl-tmp/datasheet/val_data.jsonl"）→ 落到该绝对路径，目标目录不存在时自动创建
...
def resolve_val_output(script_dir: Path) -> Path:
    """解析 VAL_OUTPUT：绝对路径原样采用（并自动创建父目录）；相对文件名拼到脚本目录下。"""
    val_setting = Path(VAL_OUTPUT).expanduser()
    val_path = val_setting if val_setting.is_absolute() else script_dir / val_setting
...
def main():
    ...
    val_path = resolve_val_output(script_dir)
    ...
    # 打印结果
    # 验证集可能写到脚本目录外（用户自定义了 VAL_OUTPUT 绝对路径时），打印完整路径更清晰
    val_display = val_path.name if val_path.parent.resolve() == script_dir else str(val_path)
    print("=" * 50)
    print("✅ 拆分完成")
    print("=" * 50)
    print(f"  原始文件:   {input_path.name} ({total} 条)")
    print(f"  训练集:     {train_path.name} ({train_count} 条)")
    print(f"  验证集:     {val_display} ({val_count} 条)")
...
> **想把验证集放到其他目录？** 把 `VAL_OUTPUT` 改成绝对路径即可，例如：
> ```python
> VAL_OUTPUT = r"D:\tts\eval\val_data.jsonl"           # Windows
> VAL_OUTPUT = "/root/autodl-tmp/datasheet/val_data.jsonl"  # Linux / AutoDL
> ```
> 脚本会自动创建缺失的父目录。`INPUT_FILE` 和 `TRAIN_OUTPUT` 仍然要求和脚本同目录（不涉及本次改动）。
