...
### 7.1 把 HF_HOME 设到系统盘 `/root/hf_cache`

HF 默认缓存在 `~/.cache/huggingface/`，也是系统盘但位置比较深。把它显式设到 `/root/hf_cache` 是为了：

- **位置干净可见**：系统盘根目录下一眼能看到
- **进系统镜像**：系统盘内容会随系统镜像一起保存
- **所有业务模型的 audio_tokenizer 引用自动命中**：业务模型加载时内部调 `AutoModel.from_pretrained("OpenMOSS-Team/MOSS-Audio-Tokenizer")`，HF 去这里查
...
env | grep -E 'HF_HOME|HF_TRANSFER'
```
...
> 这 4 条命令只操作 shell 环境变量和目录，**和 conda 环境无关**——无论在 base 还是 moss-tts 下跑都一样。`HF_HOME` 是全局环境变量，设一次后所有 conda 环境共用。下一步跑 `pip install` / `hf download` 才需要先 `conda activate moss-tts`。

### 7.2 代理策略说明

本指南**全程不开代理**：

- **下 MOSS-Audio-Tokenizer 时**：用 HF 国内镜像 `hf-mirror.com`（在第 8.1 步的下载命令前临时前置 `HF_ENDPOINT=https://hf-mirror.com`，只对那一条命令生效，不进 shell、不写 bashrc、不进镜像）
- **下 6 个业务模型时**：ModelScope 在境内，直连即可
- **日常推理/训练时**：全部文件都在本地磁盘/缓存
...
所有业务模型的 `processor_config.json` 都引用 `OpenMOSS-Team/MOSS-Audio-Tokenizer` 这个 HF 仓库 ID。把这个 repo 下到 `HF_HOME=/root/hf_cache` 后，业务模型加载时内部查 HF 缓存就能命中，**零网络调用**。

**为什么不直连 huggingface.co？** MOSS-Audio-Tokenizer 的大权重分片走 HF 新的 **Xet CAS 存储后端**（`cas-server.xethub.hf.co`），对未认证请求返回 401 / 对 AutoDL 路由抽风常导致中断。走国内镜像站 `hf-mirror.com` 完全绕过这两个问题：不需要 token、不需要代理、走国内 CDN 速度稳。

```bash
HF_ENDPOINT=https://hf-mirror.com hf download OpenMOSS-Team/MOSS-Audio-Tokenizer
```

> **为什么是命令前置而不是 `export`？** `HF_ENDPOINT=xxx command` 这种写法只让变量在这一条命令的进程里生效，**不污染当前 shell、不写 bashrc、不进系统镜像**。下完这条命令后 shell 恢复原状，符合"只有下这个模型才用到 HF"的原则。
>
...
Q: 推理时 AutoProcessor 仍然尝试连 HF 下载 MOSS-Audio-Tokenizer
A: HF 缓存里没有它。检查:
...
A: hf-mirror 由志愿者维护，偶有维护窗口或被限流。用 ModelScope 下 Audio-Tokenizer 到 /root/models/ 兜底：
   ...
   改成本地绝对路径后，HF 看是路径不是 ID，直接读本地，全流程不碰网络。
