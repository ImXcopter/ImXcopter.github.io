...
1. 修改脚本顶部的 `TARGET_TASK`（6 选 1）和 `DEFAULT_LANGUAGE`
2. **设置 `AUDIO_DIR` 为 WAV + SRT 所在文件夹的绝对路径**（例如 `r"D:\TTS_Training\raw_audio"`）。留空字符串 `""` 则默认使用脚本所在目录（保持与 VoxCPM 行为一致）
3. 可选：设置 `OUT_DIR`（切片 WAV + JSONL 的输出目录）。留空则默认写到 `AUDIO_DIR / "datasets"`
4. 将 `AUDIO_FILENAME` / `SRT_FILENAME` 改成要处理的文件名（文件名，不是路径，放在 `AUDIO_DIR` 下）
5. 运行脚本
6. 对每个音频文件依次改 `AUDIO_FILENAME` / `SRT_FILENAME` 重新运行——脚本支持追加模式（序号与 JSONL 自动续接），与 VoxCPM 行为一致

**Realtime 任务不适合这个切分流程**——它需要多轮对话数据，切片是单条 utterance。Realtime 的数据组织见本文 **第 7 节**（§五）。

### 3.4 完整代码

将以下脚本保存到**本地电脑**任意位置（例如 `D:\TTS_Training\segment_audio.py`）。**代码主体复用 VoxCPM 的 `segment_audio.py`**，仅在"写 JSONL"环节按 `TARGET_TASK` 填入对应字段，并在配置区增加 `AUDIO_DIR` / `OUT_DIR` 让音频和脚本解耦。
...
核心差异：只改了写 JSONL 这一步，切分逻辑与 VoxCPM 完全一致。
"""
...
# 目标任务：决定输出 JSONL 的字段集合
# 可选值:
#   "moss_tts"            — MOSS-TTS / MOSS-TTS-Local (不含克隆，纯 text+speech)
#   "moss_tts_cloning"    — MOSS-TTS / MOSS-TTS-Local + 单说话人克隆训练（需指定 REF_AUDIO_PATH）
#   "moss_ttsd"           — MOSS-TTSD（多人对话，reference 为 list；本脚本把所有切片都认作同一个说话人，
#                           reference=[REF_AUDIO_PATH]；多说话人请手工编辑 JSONL）
#   "moss_voice_generator"— MOSS-VoiceGenerator（需手工加 instruction 字段，后处理补）
#   "moss_sound_effect"   — MOSS-SoundEffect（用 ambient_sound 字段，文本直接作为音效描述）
TARGET_TASK = "moss_tts"

# 音频 + SRT 所在目录（绝对路径）
# 留空字符串 "" 则回退为"脚本所在目录"（保持与 VoxCPM 行为一致）
# 例如: r"D:\TTS_Training\raw_audio"  或  "/Users/me/tts/audio"
AUDIO_DIR = ""

# 输出目录：切片 WAV + JSONL 写入的位置
# 留空字符串 "" 则默认使用  <AUDIO_DIR>/<OUT_SUBDIR>
# 例如: r"D:\TTS_Training\datasheet"
OUT_DIR = ""
OUT_SUBDIR = "datasets"                 # 仅当 OUT_DIR 为空时作为 AUDIO_DIR 下的子目录名

# 当前要处理的音频 + SRT（文件名，位于 AUDIO_DIR 下）
AUDIO_FILENAME = "01.wav"
SRT_FILENAME   = "01.srt"

# 基础字段
DEFAULT_LANGUAGE = "zh"                 # zh / en / ja / ko / de / fr / ... 全家族支持 20 种
REF_AUDIO_PATH   = ""                   # 用于 moss_tts_cloning / moss_ttsd，相对 AUDIO_DIR 或绝对路径

# ─── 步骤 1：响度归一化（母带） ───
ENABLE_NORMALIZATION = True
NORMALIZATION_METHOD = "loudnorm"       # "loudnorm" 或 "peak"
TARGET_LUFS = -23.0
TARGET_TP_DB = -1.5
TARGET_LRA = 11.0
TARGET_PEAK_DB = -1.0
...
# ─── 步骤 3：前后静音填充 ───
FRONT_SILENCE_MS = 250
END_SILENCE_MS = 250                    # 推荐 ≤ 500，避免模型学到过长尾静音
TAIL_SILENCE_THRESHOLD_DBFS = -40.0

# ─── 步骤 4：切片质量守门 ───
MIN_VOICE_ENERGY_DBFS = -50.0

# ─── 母带中间文件管理 ───
KEEP_PREPROCESSED_FILE = False

# ─── 输出 JSONL 文件名 ───
OUT_JSONL_NAME = "train_raw.jsonl"

# ============================================================================
# 工具函数（与 VoxCPM 完全一致，只复制关键点，这里省略重复注释）
# ============================================================================
...
def resolve_paths() -> Tuple[Path, Path]:
    """解析 AUDIO_DIR / OUT_DIR 两个配置项；空字符串时回退到与 VoxCPM 一致的默认行为。

    返回: (audio_dir, out_dir)
      - audio_dir: AUDIO_DIR 有值则用它；否则用脚本所在目录
...
def run_ffmpeg_loudnorm(input_path: Path, output_path: Path) -> bool:
    ...
    if result.returncode != 0:
        print(f"  ❌ ffmpeg loudnorm 失败: {result.stderr[-400:]}")
        return False
    return True

def run_ffmpeg_peak_normalize(input_path: Path, output_path: Path) -> bool:
    cmd_measure = ['ffmpeg', '-hide_banner', '-i', str(input_path),
    ...
    cmd_apply = ['ffmpeg', '-y', '-hide_banner', '-loglevel', 'error',
                 '-i', str(input_path), '-af', f'volume={gain_db}dB',
...
def parse_srt(srt_path: Path) -> List[Dict[str, Any]]:
    ...
    for block in blocks:
        ...
        if not m: continue
        def ts_to_ms(h, mi, s, ms): return int(h)*3600_000 + int(mi)*60_000 + int(s)*1000 + int(ms)
        start_ms = ts_to_ms(*m.groups()[:4])
        end_ms   = ts_to_ms(*m.groups()[4:])
        text = ' '.join(l.strip() for l in lines[2:]).strip()
        entries.append({'idx': idx, 'start_ms': start_ms, 'end_ms': end_ms, 'text': text})
    return entries

def segment_audio_with_vad(master_wav: Path, srt_entries: List[Dict[str, Any]],
                           out_dir: Path, start_seq: int) -> List[Dict[str, Any]]:
    """调用 Silero VAD 在 SRT 时间轴附近精修边界，返回 [{audio, text, start_s, end_s}]"""
    import numpy as np
    import soundfile as sf
    import torch
    ...
    audio, sr = sf.read(str(master_wav), dtype='float32', always_2d=False)
    if audio.ndim > 1:
        audio = np.mean(audio, axis=1)

    def ms_to_samp(ms: int) -> int:
    ...
    for ent in srt_entries:
        expand_start = max(0, ms_to_samp(ent['start_ms'] - BUFFER_MS))
        expand_end   = min(len(audio), ms_to_samp(ent['end_ms'] + BUFFER_MS))
        segment = audio[expand_start:expand_end]
        if segment.size == 0: continue

        vad_input = torch.from_numpy(segment)
        if sr not in (8000, 16000):
            # Silero VAD 需要 16k/8k；简易降采样
            from scipy.signal import resample_poly
            target_sr = 16000
            resampled = resample_poly(segment, target_sr, sr).astype('float32')
            vad_input = torch.from_numpy(resampled)
            vad_sr = target_sr
        else:
            vad_sr = sr

        ts = get_speech_timestamps(vad_input, model, threshold=VAD_THRESHOLD, sampling_rate=vad_sr)
        if not ts:
            print(f"  [跳过 SRT {ent['idx']:>4}] VAD 未检测到语音")
            continue
        vad_start = int(ts[0]['start'] * sr / vad_sr)
        vad_end   = int(ts[-1]['end']  * sr / vad_sr)
        abs_start = expand_start + vad_start
        abs_end   = expand_start + vad_end
        clip = audio[abs_start:abs_end]

        # 能量过滤
        energy_db = measure_rms_dbfs(clip)
        if energy_db < MIN_VOICE_ENERGY_DBFS:
            print(f"  [跳过 SRT {ent['idx']:>4}] 能量 {energy_db:.1f} dBFS < 阈值")
            continue

        # 尾静音精确裁剪
        tail_samples = measure_tail_silence_samples(clip, TAIL_SILENCE_THRESHOLD_DBFS)
        if tail_samples > 0:
            clip = clip[: max(0, len(clip) - tail_samples)]

        # 前后静音填充
        front = np.zeros(ms_to_samp(FRONT_SILENCE_MS), dtype=clip.dtype)
        back  = np.zeros(ms_to_samp(END_SILENCE_MS),   dtype=clip.dtype)
        final_audio = np.concatenate([front, clip, back])

        out_name = f"{seq:04d}.wav"
        out_path = out_dir / out_name
        sf.write(str(out_path), final_audio, sr, subtype='PCM_16')
        duration_s = len(final_audio) / sr
        results.append({
            'audio_rel': f"./{out_dir.name}/{out_name}",
            'audio_abs': str(out_path.resolve()),
            'text': ent['text'],
            'duration': round(duration_s, 3),
        })
        seq += 1
        print(f"  [OK   SRT {ent['idx']:>4}] → {out_name}  time={duration_s:.2f}s")
    return results

# ============================================================================
# 写 JSONL：按 TARGET_TASK 选字段（这是 VoxCPM 不同的地方）
# ============================================================================

def build_jsonl_record(seg: Dict[str, Any]) -> Dict[str, Any]:
    """根据 TARGET_TASK 决定 JSONL 每行的字段"""
    audio_path = seg['audio_abs']
    text = seg['text']

    if TARGET_TASK == "moss_tts":
        # 最基础的 text + speech 对（见 moss_tts_delay/finetuning/README.md §2.1）
        return {"audio": audio_path, "text": text, "language": DEFAULT_LANGUAGE}

    elif TARGET_TASK == "moss_tts_cloning":
        # 单说话人克隆训练（见 §2.1 "Voice cloning / reference-conditioned training"）
        ...
        # MOSS-TTSD 用 list 形式的 reference（见 §2.2）
        # 本脚本是单说话人切分，默认把所有切片认作同一说话人
        # 多说话人场景需要手工编辑切片路径和 reference 列表
        rec = {"audio": audio_path, "text": text, "language": DEFAULT_LANGUAGE}
        if REF_AUDIO_PATH:
...
def check_prerequisites() -> bool:
    ...
    if not audio_dir.exists(): print(f"❌ AUDIO_DIR 不存在: {audio_dir}"); return False
    if not audio_path.exists(): print(f"❌ 找不到 {audio_path}"); return False
    if not srt_path.exists():   print(f"❌ 找不到 {srt_path}");   return False
    if END_SILENCE_MS < 0 or FRONT_SILENCE_MS < 0:
        print("❌ 静音时长不能为负"); return False
    if ENABLE_NORMALIZATION and NORMALIZATION_METHOD not in ("loudnorm", "peak"):
        print(f"❌ 未知 NORMALIZATION_METHOD: {NORMALIZATION_METHOD}"); return False
    if TARGET_TASK == "moss_tts_cloning" and not REF_AUDIO_PATH:
        print("❌ moss_tts_cloning 任务必须设置 REF_AUDIO_PATH"); return False
    if TARGET_TASK == "moss_ttsd" and not REF_AUDIO_PATH:
        print("⚠️  moss_ttsd 没填 REF_AUDIO_PATH，将输出 reference 为空的 JSONL")
    return True
...
def main() -> int:
    ...
    print(f"  输入 SRT:   {srt_path.name}")
    print(f"  OUT_DIR:   {out_dir}")
    print(f"  输出 JSONL: {jsonl_path}")
    print("=" * 60); print()

    # 1. 母带归一化
    prepared = prepare_master_audio(audio_path)
    if prepared is None: return 1
    master_wav, intermediates = prepared

    # 2. 解析 SRT
    srt_entries = parse_srt(srt_path)
    print(f"\n步骤 2：SRT 解析完成，共 {len(srt_entries)} 条")
    if not srt_entries: return 1

    # 3. 追加模式：读取上次序号
    last_seq = get_last_segment_number(jsonl_path)
    start_seq = last_seq + 1
    print(f"步骤 3：本次从序号 {start_seq:04d} 开始")

    # 4. 切分
    print(f"\n步骤 4：Silero VAD 切分")
    segs = segment_audio_with_vad(master_wav, srt_entries, out_dir, start_seq)
    print(f"  切分完成，共输出 {len(segs)} 个切片")

    # 5. 写 JSONL
    records = [build_jsonl_record(s) for s in segs]
    append_jsonl(records, jsonl_path)
    print(f"\n步骤 5：JSONL 追加写入完成 → {jsonl_path}")
    print(f"  本次新增 {len(records)} 条，累计 {last_seq + len(records)} 条")
...
- Python 3.10+（和 VoxCPM 教程的本地虚拟环境相同，见 `VoxCPM/2026-4-16-voxcpm-finetune-all-tutorial.md`）
- `pip install soundfile scipy torch` （CPU 版 torch 就够，不用 CUDA）
- 系统装了 `ffmpeg`，`ffmpeg` 在 PATH 里能直接调用
- 首次运行脚本时，`torch.hub.load('snakers4/silero-vad', ...)` 会从 GitHub 拉取 VAD 模型 → 家庭/办公网络通常直连即可，如果 GitHub 访问受限，配好本机代理（VPN/Clash）再跑

**Windows PowerShell 示例**：

```powershell
# 1. 激活你本地的音频处理虚拟环境（名字按你自己的来）
# 例如: D:\Project\TTS_ASR_Tools\VoxCPM_FT\.venv\Scripts\Activate.ps1
& "D:\Project\TTS_ASR_Tools\VoxCPM_FT\.venv\Scripts\Activate.ps1"

# 2. 脚本放在任意位置，跑就行（脚本里 AUDIO_DIR 决定从哪里读音频）
...
```bash
source ~/venvs/audio-tools/bin/activate
