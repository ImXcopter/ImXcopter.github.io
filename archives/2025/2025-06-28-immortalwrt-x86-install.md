...
方式 1 下载：<https://firmware-selector.immortalwrt.org/>

方式 2 下载：<https://downloads.immortalwrt.org/releases/24.10.2/targets/x86/64/>

![](/static/2025/2025-06-28-immortalwrt-x86-install_001.png)

截至发文的 immotalwrt 的最新版本为 24.10.2。

这里需要说明下，x86 总体有三个镜像类型，常用 2 个类型也就是 ext4 和 squashfs，这两个版本又分了 efi 版本和非 efi 版本。

- ext4 和 squashfs 区别：squashfs 可以使用类似应路由的重置还原功能，ext4 则没用（也可以用其他方方法达到相似效果）
...
使用 U 盘制作一个 PE 系统，注意，U盘的内容会被格式化，注意备份 U 盘数据。推荐使用 WEPE，具体制作步骤请自行搜索。
...
下载写盘工具 [physdiskwrite](https://m0n0.ch/wall/physdiskwrite.php)，然后打开 cmd 窗口【开始→cmd】：
...
写入完成后重启，bois 设置开机启动项为硬盘，并拔掉 U 盘。启动好以后即可享用 immotalwrt。

## 提示

默认 Lan 地址 `192.168.1.1`，默认账号：`root` 默认密码：空
